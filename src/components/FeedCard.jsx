    ...
    <article className="flex p-3 md:p-6 gap-2 md:gap-3 transition-colors border-b border-gray-200 dark:border-gray-800 w-full max-w-2xl">
      ...
      <div className="shrink-0">
        <img
          src={
            post?.user?.avatar ||
            `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(post?.user?.name)}`
          }
          alt="pfp"
          onClick={goToUser}
        ...
        <div className="flex items-center justify-between mx-2">
          <div className="flex items-center px-2 py-1.5 rounded-lg transition-colors group">
            <button onClick={() => setLiked(!liked)} title={liked ? "Unlike" : "Like"}>
              <Heart className={`size-5 cursor-pointer hover:text-red-500 ${liked ? "fill-red-500 text-red-500" : ""}`} />
            </button>
          ...
          <div className="flex items-center px-2 py-1.5 rounded-lg transition-colors group">
            <button onClick={() => setSaved(!saved)} title={saved ? "Unsave" : "Save"}>
              <Bookmark className={`size-5 cursor-pointer hover:text-yellow-500 ${saved ? "fill-yellow-500 text-yellow-500" : ""}`} />
            </button>
          </div>
        </div>
      </div>
